...
def decode(scores, _tokens):
    ...
    for i in range(1, len(scores)):
        ...
        if dL >= 0: # left word boundary
            output.append(i)
        if dR >= 0: # right word boundary
            output.append(i + scores[i][2])
    output.append(len(scores))
    output = sorted(set(output))
    output = [_tokens[i:j] for i, j in zip(output[:-1], output[1:])]
    return output

def tokenize(model):
    output = []
    ngram_sizes = sorted(set([1, *NGRAM_SIZES]))
    ...
    for line in fo:
        ...
        for i in range(len(tokens)):
            ...
            scores[i] = max(_scores, key = lambda x: (sum(x[:2]), x[2]))
            if DEBUG:
                w = sep.join(tokens[i:i + scores[i][2]])
                print("score[%d] = " % i, (*scores[i][:2], w))

        _output = decode(scores, _tokens)
